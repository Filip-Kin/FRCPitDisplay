function generateQueue(update) {
    ...
    getAllMatches().then(json => {
        ...
        if (highestMatch === 0) { // Haven't started yet
            getStats().then(stats => {
                if (stats.qual === null) {
                    return qt.innerHTML = '<div class="center"><i class="material-icons">timer</i> Waiting for event to start</div>'
                }
                if (stats.qual.ranking.rank === null) { // Apparently ranking is null once the event kickoff happens
                    qt.innerHTML = `
                    <tr><td colspan=8><strong id="queuing">Now Queuing First 3 Matches</strong></td></tr>
                    <tr>
                        <th>Match</th>
                        <th>ETA</th>
                        <th colspan=3 style="color:Red;">Red Alliance</th>
                        <th colspan=3 style="color:Blue;">Blue Alliance</th>
                    </tr>`;
                    generateQueueTable(json, highestMatch, qt);
                } 
            });
        } else {
            qt.innerHTML = `
            ...
            <tr>
                ...
                <th>ETA</th>
                <th colspan=3 style="color:Red;">Red Alliance</th>
                <th colspan=3 style="color:#537bff;">Blue Alliance</th>
            </tr>`;
            generateQueueTable(json, highestMatch, qt);
        }
    }).catch(err => {
...
function generateQueueTable(json, highestMatch, qt) {
    let nextRows = [];
    json = json.filter((value) => {
        return (value.match_number >= highestMatch && value.match_number < highestMatch+4); // Pick the next 3 matches
    });
    json.forEach(match => {
